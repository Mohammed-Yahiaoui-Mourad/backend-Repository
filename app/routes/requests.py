...
@router.get("/active")
def get_active_requests(session: Session = Depends(get_session)):
    """Get all active blood requests"""
    statement = select(BloodRequest).where(
        BloodRequest.status.in_(["pending", "active", "partially_fulfilled"])
    )
    requests = session.exec(statement).all()
    return {
        "success": True,
        "data": requests,
        "message": "Active requests loaded successfully"
    }

@router.get("/search")
def search_requests_by_blood_type(blood_type: str, session: Session = Depends(get_session)):
    """Search for requests by blood type"""
    statement = select(BloodRequest).where(
    ...
    return {
        "success": True,
        "data": requests,
        "message": f"Requests for blood type {blood_type} loaded successfully"
    }
...
def get_nearby_requests(
    ...
    from app.scheduler import calculate_distance
    ...
    return {
        "success": True,
        "data": nearby,
        "message": f"Found {len(nearby)} requests within {radius}km"
    }
...
def create_blood_request_test(
    ...
    """Temporary test endpoint: creates a blood request without authentication.
    Uses an existing DB user as the creator or creates a lightweight dev user.
    Intended for local testing only — remove before production."""
    # Ensure there is a user to assign as creator
    ...
    # Reuse create logic from create_blood_request
    valid_types = {"A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"}
    if request_data.blood_type not in valid_types:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    # Broadcast and trigger matching if needed
    from datetime import datetime
    req_dict = {
        "id": db_request.id,
        "created_by_id": db_request.created_by_id,
        ...
        "hospital_longitude": db_request.hospital_longitude,
        "urgency_level": db_request.urgency_level,
        "needed_by": db_request.needed_by.isoformat() if hasattr(db_request.needed_by, 'isoformat') else db_request.needed_by,
        "status": db_request.status,
        "created_at": db_request.created_at.isoformat() if hasattr(db_request.created_at, 'isoformat') else db_request.created_at
    }
    broadcast_request_update("request_created", req_dict)
